            ...
            () => {
                ...
                items.forEach(item => {
                    ...
                    const imgs = Array.from(item.querySelectorAll('img'))
                        .map(i => i.getAttribute('src') || i.getAttribute('data-src') || i.getAttribute('srcset') || '')
                        .map(s => (s.split(' ')[0] || '').trim())
                        .filter(s => s && !s.startsWith('data:') && !s.includes('placeholder') && !s.includes('blank.gif'));
                    const images = Array.from(new Set(imgs));
                    // --- localisation (commune / postcode) ---
                    const locEl = item.querySelector(
                        '[class*="location" i], [class*="Location" i], [class*="seller-location" i], [class*="city" i], [class*="postcode" i]'
                    );
                    const location = locEl ? locEl.innerText.trim() : '';
                    // --- distance affichee par 2ememain ("a 12 km") ---
                    const fullText = (item.innerText || '');
                    const distMatch = fullText.match(/à\\s+(\\d+(?:[\\.,]\\d+)?)\\s*km/i)
                        || fullText.match(/(\\d+(?:[\\.,]\\d+)?)\\s*km\\b/i);
                    const distance_display = distMatch ? distMatch[0].trim() : '';
                    const distance_km = distMatch ? parseFloat(distMatch[1].replace(',', '.')) : null;
                    // --- description courte ---
                    const descEl = item.querySelector('[class*="description" i], [class*="Description" i], p');
                    let description = descEl ? descEl.innerText.trim() : '';
                    if (description.length > 240) description = description.substring(0, 237) + '...';
                    // --- date / "aujourd'hui" / "hier" ---
                    const dateEl = item.querySelector('[class*="date" i], [class*="Date" i], time');
                    const date_label = dateEl ? dateEl.innerText.trim() : '';
                    out.push({
                        priceRaw, title, link, images,
                        location, distance_display, distance_km,
        ...
        for raw in ads:
            ...
            if ok:
                ...
                accepted.append({
                    ...
                    "category": category,
                    "source_url": url,
                    # nouveaux champs (Lot 2, 2026-04-29)
                    "images": images,
                    "image_main": images[0] if images else "",
                    "location": raw.get("location") or "",
                    "distance_display": raw.get("distance_display") or "",
                    "distance_km": raw.get("distance_km"),
                    "description": raw.get("description") or "",
                    "date_label": raw.get("date_label") or "",
                })
