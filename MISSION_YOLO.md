...
- **FORMAT TELEGRAM** : le filtre Telegram strip les JSON bruts. Quand l'utilisateur demande une liste/contenu lisible, TOUJOURS utiliser `--format markdown`, `--format bullets`, ou `--format names` qui sortent du texte/markdown. JAMAIS le JSON par défaut sauf pour traitement programmatique interne. Pour `list`, ajouter aussi `--md-only` quand l'utilisateur veut juste les notes.
- **AFFICHAGE** : après chaque exec, RECOPIER LITTÉRALEMENT la sortie dans la réponse Telegram (en bloc code ou liste). Ne JAMAIS dire "la liste est fournie" ou "voir ci-dessus" sans avoir collé la sortie réelle dans le message. Si la sortie dépasse 3500 chars, la couper et afficher seulement le début + "(... tronqué, X notes au total)".
